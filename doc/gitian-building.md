...
Connect to `localhost`, port `29092` (or the port configured when installing the VM).
On Windows you can use putty[1] and WinSCP[2].

For example to connect as `root` from a Linux command prompt use

    $ ssh root@localhost -p 29092
    The authenticity of host '[localhost]:29092 ([127.0.0.1]:29092)' can't be established.
    ECDSA key fingerprint is 8e:71:f9:5b:62:46:de:44:01:da:fb:5f:34:b5:f2:18.
    Are you sure you want to continue connecting (yes/no)? yes
    Warning: Permanently added '[localhost]:29092' (ECDSA) to the list of known hosts.
...
Clone the git repositories for cgen and gitian and then checkout the cgen version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cgen-crypto/cgen.git
cd cgen
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cgen repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the cgen repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cgen/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/cgen-crypto/cgen
...
For example:
```bash
URL=https://github.com/crowning-/cgen.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cgen=${COMMIT} --url cgen=${URL} ../cgen/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cgen=${COMMIT} --url cgen=${URL} ../cgen/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cgen=${COMMIT} --url cgen=${URL} ../cgen/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cgen-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cgen-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cgen-build.assert
```
...
[cgen/gitian.sigs](https://github.com/cgen-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
