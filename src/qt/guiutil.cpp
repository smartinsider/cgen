...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CGen address (e.g. %1)").arg("XEBxEjGXmJxwpXxXAJqeXztdUR4MsZZJsE"));
#endif
...
{
    // Convert cgen:// to cgen:
    //
    //    Cannot handle this later, because cgen:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open cgen.conf with the associated application */
...
{
    return GetAutostartDir() / "cgen.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a cgen.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the CGen app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add CGen app to startup item list
