...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_CGen4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_CGen, SIGNAL(clicked()), this, SLOT(openMyAddresses()));
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  ui->label_lcolat->setText("100 000 Coins");
  ui->label_mcolat->setText("500 000 Coins");
  ui->label_fcolat->setText("1 000 000 Coins");
...
double BlockRewardCGen =  static_cast<double>(BlockReward/COIN);
//int64_t CGenSupply = chainActive.Tip()->nMoneySupply / COIN;
...
ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardCGen));
//ui->label_CGenSupply_value->setText(QString::number(CGenSupply));
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeCgenAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCgenAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeCgenAmount = strAnonymizeCgenAmount.remove(strAnonymizeCgenAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCgenAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeCgenAmount * COIN) nMaxToAnonymize = nAnonymizeCgenAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeCgenAmount * COIN) {
        // ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
        //                                       .arg(strAnonymizeCgenAmount));
        strAnonymizeCgenAmount = strAnonymizeCgenAmount.remove(strAnonymizeCgenAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCgenAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
