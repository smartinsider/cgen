        ...
        if (!fProofOfStake) {
            ...
            if (txNew.vout.size() > 1) {
                ...
                txNew.vout[1].scriptPubKey = CScript()
                                             << ParseHex(Params().DevPubKey1().c_str())
                                             << OP_CHECKSIG;
                txNew.vout[1].nValue = GetDevFundPayment1(
                    pindexPrev->nHeight, GetBlockValue(nHeight));


                txNew.vout[2].scriptPubKey = CScript()
                                             << ParseHex(Params().DevPubKey2().c_str())
                                             << OP_CHECKSIG;
                txNew.vout[2].nValue = GetDevFundPayment2(
                    pindexPrev->nHeight, GetBlockValue(nHeight));
                

                txNew.vout[0].nValue = GetBlockValue(nHeight) + nFees - GetDevFundPayment1(
                    pindexPrev->nHeight, GetBlockValue(nHeight)) - GetDevFundPayment2(
                    pindexPrev->nHeight, GetBlockValue(nHeight));
