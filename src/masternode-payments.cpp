...
CAmount CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, CAmount block_value, bool fProofOfStake)
{
    ...
    CAmount devFund1 = GetDevFundPayment1(pindexPrev->nHeight, blockValue);
    CAmount devFund2 = GetDevFundPayment2(pindexPrev->nHeight, blockValue);
...
/* */
        ...
        txNew.vout[i].scriptPubKey = CScript()
			<< ParseHex(Params().DevPubKey1().c_str()) << OP_CHECKSIG;
	txNew.vout[i].nValue = devFund1;

        txNew.vout.resize(i + 2);
        txNew.vout[i + 1].scriptPubKey = CScript()
			<< ParseHex(Params().DevPubKey2().c_str()) << OP_CHECKSIG;
	txNew.vout[i + 1].nValue = devFund2;
...
/*
		if (fProofOfStake) {
			...
			txNew.vout[i].nValue = mn_payments_total;			
			txNew.vout.resize(i + 2);
			txNew.vout[i + 1].scriptPubKey = CScript()
					<< ParseHex(Params().DevPubKey1().c_str()) << OP_CHECKSIG;
			txNew.vout[i + 1].nValue = devFund1;
		} else {
			txNew.vout.resize(3);
			...
			txNew.vout[2].scriptPubKey = CScript()
					<< ParseHex(Params().DevPubKey1().c_str()) << OP_CHECKSIG;
			txNew.vout[2].nValue = GetDevFundPayment1(pindexPrev->nHeight,
					blockValue);
		}
		CTxDestination address1;
		ExtractDestination(payee, address1);
...
*/
    ...
    //return mn_payments_total + devFund1 + devFund2;
    return mn_payments_total + devFund1 + devFund2;
